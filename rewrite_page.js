...
type Message = { id: string; role: 'user' | 'assistant'; content: string; attachments?: any[]; action?: string };
  ...
  const handleChatSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const apiMessages = newMessages.map(m => ({
        role: m.role,
        content: m.content,
        experimental_attachments: m.role === 'user' && m.attachments ? base64Attachments : undefined
      }));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              return (
                <div key={m.id} className={\`flex gap-3 \${isUser ? "flex-row-reverse" : ""}\`}>
                  ...
                  <div className={\`px-4 py-2 rounded-2xl max-w-[80%] whitespace-pre-wrap text-sm \${isUser ? "bg-indigo-600 text-white rounded-tr-none" : "bg-white border text-slate-800 shadow-sm rounded-tl-none"}\`}>
                    ...
                    {!isUser && isLastMsg && m.action === 'REQUEST_LOCATION' && (
                      <div className="mt-3">
                        <Button 
                          type="button"
                          onClick={() => { setInput("My location is: "); document.getElementById("chat-input")?.focus(); }}
                          className="bg-white text-indigo-600 hover:bg-slate-50 border border-indigo-200 shadow-sm"
                          size="sm"
                        >
                          <MapPin className="h-4 w-4 mr-2" /> Provide Location
                        </Button>
                      </div>
                    )}

                    {!isUser && isLastMsg && m.action === 'REQUEST_IMAGE' && (
                      <div className="mt-3">
                        <Button 
                          type="button"
          ...
          <CardFooter className="border-t p-3 bg-slate-50">
            <form onSubmit={handleChatSubmit} className="flex gap-2 w-full flex-wrap md:flex-nowrap">
              ...
              <div className="flex-1 flex gap-2 w-full">
                <Input id="chat-input" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Describe the issue..." className="flex-1 bg-white" disabled={isLoading} />
        ...
        <Card className="shadow-lg border-slate-200 h-fit sticky top-6">
          ...
          <CardContent className="p-4 space-y-4">
            ...
            <div>
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Details (Summary)</p>
